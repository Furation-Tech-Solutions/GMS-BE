...
export interface UpdateBookingRequestUsecase {
  execute: (
    requestID: string,
    requestData: BookingRequestEntity
  ) => Promise<Either<ErrorClass, BookingRequestEntity>>;
}

export class UpdateBookingRequest implements UpdateBookingRequestUsecase {
  ...
  async execute(
    ...
    return await this.bookingRequestRepository.updateBookingRequest(requestID, requestData); // Change to updateBookingRequest
  }
}
