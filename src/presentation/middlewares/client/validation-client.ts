...
const clientValidator = (input: ClientInput, isUpdate: boolean = false) => {
  const clientSchema = Joi.object<ClientInput>({
    ...
    phone: isUpdate
      ? Joi.string().length(10).pattern(/^[0-9]+$/).optional().trim().messages({
      ...
      : Joi.string().length(10).pattern(/^[0-9]+$/).required().trim().messages({
        "string.length": "Phone number should have exactly 10 digits",
        "string.pattern.base": "Phone number should contain only digits",
        "any.required": "Phone number is required",
      }),
    ...
    workPhone: isUpdate
      ? Joi.string().length(10).pattern(/^[0-9]+$/).optional().trim().messages({
        ...
        "string.pattern.base": "Work phone number should contain only digits",
      })
      : Joi.string().length(10).pattern(/^[0-9]+$/).optional().trim().messages({
        "string.length": "Work phone number should have exactly 10 digits",
        "string.pattern.base": "Work phone number should contain only digits",
    ...
    contactInfoVisibilityOnlyToSuperUser: isUpdate
      ? Joi.boolean().optional().default(false) : Joi.boolean().optional().default(false),

    birthDate: isUpdate ? Joi.date().optional() : Joi.date().optional(),

    anniversaryDate: isUpdate ? Joi.date().optional() : Joi.date().optional(),

    visits: isUpdate ? Joi.number().integer().optional().default(0) : Joi.number().integer().optional().default(0),

    spends: isUpdate ? Joi.number().optional().default(0) : Joi.number().optional().default(0),

    gender: isUpdate ? Joi.string()
